"""
3×3 Cross-Dataset Generalization Matrix — Feature Group Edition
================================================================
Project : Speech-Based Parkinson's Disease Detection (BE Capstone)
Author  : Ruchit Das (22AM1084)

Derived from: scripts/pipeline/dataset_matrix.py
Stage 2 addition: --feature_group CLI argument filters the ComParE feature
space to a single semantic group before any model fitting. This answers:

    "Do voice-quality features (jitter/shimmer/HNR) generalise better
     across languages than MFCCs?"

USAGE:
    # Full run (all ComParE features — identical to original dataset_matrix.py)
    python 03_group_matrix.py

    # Group run — filter to a semantic group defined in groups.json
    python 03_group_matrix.py --feature_group mfcc
    python 03_group_matrix.py --feature_group jitter
    python 03_group_matrix.py --feature_group voicing

GROUP FILTER:
    Groups are defined in stage_2_feature_grouping/group_definitions/groups.json.
    Each group maps to a list of column-name prefix patterns (case-insensitive).
    The filter is applied BEFORE any model fitting — no leakage possible.
...
NORMALIZATION:
    ...
    Scaler is INSIDE every Pipeline — never fit on full data.
...
OUTPUTS:
    stage_2_feature_grouping/results/matrix_<group>_<TIMESTAMP>/
        run_log.txt
        matrix_auc.csv
        matrix_auc_ci_lo.csv / matrix_auc_ci_hi.csv
        cascade_results.csv
        subanalysis_results.csv
        matrix_plots.png
"""
...
BASE_OUTDIR = os.path.join(BASE, "stage_2_feature_grouping", "results")
GROUPS_JSON_PATH = os.path.join(
    BASE, "stage_2_feature_grouping", "group_definitions", "groups.json"
)
...
CASCADE_THRESH = {
    "L1 Logistic Regression": 0.70,
    "Constrained RF": 0.65,
    # L2 SVM (Linear) always outputs (Stage 3, no threshold)
}
...
def parse_group_columns(
    ...
    """
    Filter feature column names to those belonging to a semantic group.

    Uses groups.json which maps group_name → list of column-name prefix patterns.
    A column is included if its name starts with any of the group's prefix patterns
    (case-insensitive match).

    Args:
        group_name      : Key from groups.json (e.g. 'mfcc', 'jitter', 'shimmer')
        groups_json_path: Absolute path to groups.json
        all_feat_names  : Full list of feature column names extracted from the CSV

    Returns:
        List of column names whose names begin with one of the group's prefixes.

    Raises:
        FileNotFoundError: if groups.json does not exist at groups_json_path
        ValueError       : if group_name is not a key in groups.json
    """
    if not os.path.exists(groups_json_path):
        raise FileNotFoundError(
            f"groups.json not found at: {groups_json_path}\n"
            f"Expected: stage_2_feature_grouping/group_definitions/groups.json"
        )
    ...
    if group_name not in groups:
        ...
        raise ValueError(
            f"Feature group '{group_name}' not found in groups.json.\n"
            f"Available groups: {available}"
        )

    prefixes = [p.lower() for p in groups[group_name]]
    matched = [c for c in all_feat_names if any(c.lower().startswith(p) for p in prefixes)]
    return matched
...
# CLI ARGS
# ══════════════════════════════════════════════════════════════
def _load_group_choices() -> List[str]:
    """Load valid group names from groups.json for argparse choices validation."""
...
_valid_groups = _load_group_choices()

parser = argparse.ArgumentParser(
    description=(
        "3×3 Cross-Dataset Generalization Matrix (PC-GITA ES ↔ VOICE_DATASET EN).\n"
        "Optionally restrict features to one semantic ComParE group via --feature_group."
    ),
...
parser.add_argument(
    ...
    type=str,
    default=None,
    choices=_valid_groups if _valid_groups else None,
    metavar="GROUP",
    help=(
        f"Filter ComParE features to a semantic group defined in groups.json. "
        f"Available: {_valid_groups}. "
        f"Omit to use all features (full ComParE run)."
    ),
)
...
# ══════════════════════════════════════════════════════════════
# OUTPUT SETUP  (after argparse so group name can enter dirname)
# ══════════════════════════════════════════════════════════════
TIMESTAMP = datetime.now().strftime("%Y%m%d_%H%M%S")
group_suffix = f"_{args.feature_group}" if args.feature_group else "_full"
RUN_DIR = os.path.join(BASE_OUTDIR, f"matrix{group_suffix}_{TIMESTAMP}")
...
def make_model(name):
    ...
    base = {
        ...
        "Constrained RF": RandomForestClassifier(
            n_estimators=100,
            max_depth=3,
            min_samples_leaf=10,
            class_weight="balanced",
...
def run_within_cv(X, y, groups, model_name):
    ...
    Scaler fitted on each training fold separately (enforced by Pipeline).
...
# 1. LOAD
# ══════════════════════════════════════════════════════════════
feature_group_label = f"[group={args.feature_group}]" if args.feature_group else "[all features]"
section(f"3×3 Cross-Dataset Matrix — ComParE_2016 8kHz PD Detection  {feature_group_label}")
log.info(f"  Run   : matrix{group_suffix}_{TIMESTAMP}")
...
X_all_df = df.drop(columns=drop_cols).select_dtypes(include=[np.number])
feat_names = X_all_df.columns.tolist()

# ── Feature group filter ──────────────────────────────────────
# Applied to feat_names and X_all_df BEFORE conversion to numpy and BEFORE
# any model fitting. Scaler/imputer live inside Pipeline; VarianceThreshold
# is not used in this script. Subject_id/label/dataset already excluded
# above via drop_cols — only the feature space is narrowed here.
if args.feature_group:
    group_cols = parse_group_columns(args.feature_group, GROUPS_JSON_PATH, feat_names)
    if len(group_cols) == 0:
        log.error(
            f"  [ERROR] No columns matched group '{args.feature_group}'. "
            f"Check prefix patterns in: {GROUPS_JSON_PATH}"
        )
        sys.exit(1)
    group_col_set = set(group_cols)
    feat_names = [c for c in feat_names if c in group_col_set]
    X_all_df = X_all_df[feat_names]
    log.info(
        f"  Feature group filter: '{args.feature_group}' → {len(feat_names)} columns retained"
    )
...
# ══════════════════════════════════════════════════════════════
MAIN_MODEL = "L1 Logistic Regression"
...
for train_name, test_name in cross_pairs:
    ...
    stage_models = {}
    for mname in ["L1 Logistic Regression", "Constrained RF", "L2 SVM (Linear)"]:
    ...
    stage_names = ["L1 Logistic Regression", "Constrained RF", "L2 SVM (Linear)"]
...
# String-based sub-grouping within the active feature space (feat_names).
# When --feature_group is active, feat_names is already filtered — sub-groups
# are naturally subsets of that group, and "all" refers to the group total.
prosody_feats = [
...
# Dynamic label so "All ComParE features" is accurate when group-filtered
all_feats_label = (
    f"All '{args.feature_group}' features" if args.feature_group else "All ComParE features"
)
...
if len(prosody_feats) == 0 and len(spectral_feats) == 0:
    log.warning(
        "  ComParE feature names are opaque indices — "
        "splitting into equal thirds as proxy groups."
    )
    n = len(all_feats)
...
for subset_name, feat_subset in subsets.items():
    ...
    if not feat_idx:
        log.warning(
            f"  ── {subset_name} (0 features) — skipped "
            f"(no features in active set matched this sub-group)"
        )
        continue
    ...
    for train_name, test_name in cross_pairs:
        ...
        # Run Constrained RF as standard comparison model
        pipe = make_model("Constrained RF")
...
group_title_str = f"  |  Feature group: {args.feature_group}" if args.feature_group else ""
fig = plt.figure(figsize=(24, 20))
fig.patch.set_facecolor("#f8fafc")
fig.suptitle(
    f"Cross-Dataset Generalization Matrix  |  "
    f"PC-GITA (ES) + VOICE_DATASET (EN)  |  "
    f"Primary: {MAIN_MODEL}{group_title_str}  |  "
...
ax1.set_title(
    f"3×3 AUC Matrix ({MAIN_MODEL}){group_title_str}\n"
...
colors = {
    "L1 Logistic Regression": "#2563EB",
    "L2 SVM (Linear)": "#f59e0b",
    "Constrained RF": "#DC2626",
}
...
# Dynamic palette — avoids KeyError when subset names differ from original hardcoded dict
_sub_palette = ["#ef4444", "#8b5cf6", "#2563EB", "#f59e0b", "#16a34a"]
sub_colors = {name: _sub_palette[i % len(_sub_palette)] for i, name in enumerate(subset_nms)}
...
legend_els = [
    plt.Rectangle(
        ...
        1,
        1,
        fc=stage_colors["L1 Logistic Regression"],
        label=f"Stage 1: L1-LR (≥70%)",
    ),
    plt.Rectangle(
        (0, 0), 1, 1, fc=stage_colors["Constrained RF"], label=f"Stage 2: RF (≥65%)"
    ),
    plt.Rectangle(
        (0, 0), 1, 1, fc=stage_colors["L2 SVM (Linear)"], label=f"Stage 3: SVM (all)"
    ),
]
