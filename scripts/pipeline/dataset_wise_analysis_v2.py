...
def make_pipeline(model):
    ...
    If KBEST_K is set, adds SelectKBest(f_classif, k=KBEST_K) after scaling.
    SelectKBest fits on training data inside each fold — no leakage."""
    steps = [
        ("imputer", SimpleImputer(strategy="median")),
        ("scaler", StandardScaler()),
    ]
    if KBEST_K is not None:
        steps.append(("selector", SelectKBest(f_classif, k=KBEST_K)))
...
def load_full_data(data_file):
    ...
    logger.info(
        f"  PD={df['label_binary'].sum()}  HC={(df['label_binary'] == 0).sum()}"
    )
...
def train_all_models(X_train, X_test, y_train, y_test):
    ...
    models_config = {
        "LogisticRegression": LogisticRegression(
            max_iter=1000, C=1, random_state=RANDOM_STATE, class_weight="balanced"
        ),
        "SVM": SVC(
            kernel="rbf",
            C=10,
    ...
    try:
        ...
        try:
            # Try GPU first
            pipe = make_pipeline(
                xgb.XGBClassifier(**xgb_params, tree_method="hist", device="cuda")
            )
            pipe.fit(X_train, y_train)
        except:
            # Fallback to CPU
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate PD Detection Models on specific Feature CSVs."
    )
    parser.add_argument(
        "--data_file",
        type=str,
        required=True,
        help="Path to the training CSV (e.g., ../features/training_sustained_a_STABLE.csv)",
    )
    parser.add_argument(
        "--kbest",
        type=int,
        default=None,
        help="Number of top features via SelectKBest(f_classif). None = use all features.",
    )
    ...
    if KBEST_K is not None:
        logger.info(
            f"K-Best mode: SelectKBest(f_classif, k={KBEST_K}) applied inside every Pipeline."
        )
    ...
    if KBEST_K is not None:
        base_name = f"{base_name}_kbest{KBEST_K}"
    TIMESTAMP = datetime.now().strftime("%Y%m%d_%H%M%S")

    # We navigate up one directory because this script is executed from inside 'scripts/'
    base_project_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    OUTPUT_DIR = os.path.join(
        base_project_dir, "results", "evaluations", f"evaluation_{base_name}_{TIMESTAMP}"
    )
    ...
    for dataset_name in datasets:
        ...
        if n_splits < 2:
            logger.warning(
                f"  {dataset_name}: not enough subjects for GroupKFold — skipping"
            )
            continue
        ...
        for m in model_names_self:
            auc_vals = [
                f["results"][m].get("auc", 0)
                for f in all_fold_results_self
                if m in f["results"]
            ]
            if auc_vals:
                logger.info(
                    f"    {m}: {np.mean(auc_vals):.4f} ± {np.std(auc_vals):.4f}"
                )
        ...
        logger.info(
            f"  🏆 Best Model: {best_model_name} (AUC={results[best_model_name].get('auc', 0):.4f})"
        )
    ...
    if len(datasets) > 1:
        ...
        for train_dataset in datasets:
            for test_dataset in datasets:
                ...
                results, y_test_eval = train_all_models(
    ...
    logger.info(
        f"  Combined Data: {len(df)} rows | Subjects={n_unique_all} | Folds={n_splits_all}"
    )

    cv_all = StratifiedGroupKFold(
        n_splits=n_splits_all, shuffle=True, random_state=RANDOM_STATE
    )
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(
        cv_all.split(X_all, y_all, groups_all)
    ):
        ...
        all_fold_results.append(
            {"fold": fold_idx, "results": results, "y_test": y_test_eval}
        )
    ...
    for key, val in all_results.items():
        json_results[key] = {
            k: {
                m: float(v)
                for m, v in d.items()
                if m not in ("pred", "proba") and v is not None
            }
    ...
    pivot_auc = summary_df.pivot_table(
        values="AUC", index="Analysis", columns="Model", aggfunc="mean"
    )
    ...
    # ── Heatmap 2: MCC ──
    pivot_mcc = summary_df.pivot_table(
        values="MCC", index="Analysis", columns="Model", aggfunc="mean"
    )
    plt.figure(figsize=(10, 6))
    sns.heatmap(
        pivot_mcc,
