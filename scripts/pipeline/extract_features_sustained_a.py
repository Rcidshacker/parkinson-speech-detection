"""
Sustained Vowel "a" Feature Extraction Pipeline
================================================
...
OUTPUT:
    features/features_sustained_a.csv          -- clean, deduped feature matrix
    features/dedup_report_sustained_a.csv      -- log of removed duplicate pairs
    logs/extraction_sustained_a_<ts>.log       -- full run log

HOW TO RUN:
    venv\\Scripts\\activate
    python extract_features_sustained_a.py
"""
...
OUTPUT_CSV   = os.path.join(BASE, "features", "handcrafted", "features_sustained_a.csv")
DEDUP_REPORT = os.path.join(BASE, "features", "handcrafted", "dedup_report_sustained_a.csv")
...
F0_MIN           = 80
F0_MAX           = 300
FORMANT_MAX_FREQ = 5500

# Feature quality thresholds
# Drop features whose variance across all subjects falls below this value.
# At 8 kHz + N_FFT=256 settings, certain features (e.g. raw mel mean,
# some jitter columns on unvoiced frames) collapse to near-constants and
# carry no discriminative signal. Threshold of 0.001 removes those safely.
...
LOG_FILE  = os.path.join(LOG_DIR, f"extraction_sustained_a_{TIMESTAMP}.log")
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("  SUSTAINED VOWEL 'A' FEATURE EXTRACTION PIPELINE")
    logger.info("=" * 80)
