...
THE CORE RESEARCH QUESTION:
    "Does a model trained on Spanish PD speech detect PD in Italian speech
     it has never seen before — and vice versa?"

3×3 MATRIX:
    Rows (Train) : D1=PC-GITA(ES),  D2=VOICED(IT),  D1+D2=Combined
    Cols (Test)  : D1=PC-GITA(ES),  D2=VOICED(IT),  D1+D2=Combined

    Diagonal     → Within-dataset: 5-fold StratifiedGroupKFold (CV)
    Off-diagonal → Cross-lingual: train on full set, test on full other set
                   (true zero-overlap cross-lingual evaluation)
...
FEATURE SUB-ANALYSES (per supervisor: feature-category-wise):
    Run 1: Biomarker features only   (9 features)
    Run 2: Spectral features only    (MFCC + spectral + energy, 42 features)
    Run 3: All features combined     (77 features)
    Answers: which feature category drives the cross-lingual signal?

BOOTSTRAP CIs: 1000 iterations on every matrix cell.

OUTPUTS:
    results/matrix_<TIMESTAMP>/
        run_log.txt
        matrix_auc.csv              — 3×3 AUC matrix
        matrix_auc_ci.csv           — 3×3 CI bounds
        cascade_results.csv         — cascade stage breakdown
        subanalysis_results.csv     — biomarker vs spectral vs all
        matrix_plots.png            — heatmaps + cascade + sub-analysis

HOW TO RUN:
    venv\\Scripts\\activate
    python scripts\\dataset_matrix.py
"""
...
ALL_META = ["file","label","label_binary","disease_label","multiclass_label",
            "language","dataset","speech_type","speaker_id","subject_id",
            "gender","meds_status","updrs_total","moca_score"]
...
RUN_DIR   = os.path.join(BASE_OUTDIR, f"matrix_{TIMESTAMP}")
...
def run_within_cv(X, y, groups, model_name):
    ...
    proba = cross_val_predict(pipe, X, y, cv=cv, groups=groups,
...
# 1. LOAD
# ══════════════════════════════════════════════════════════════
section("3×3 Cross-Dataset Matrix — Cross-Lingual PD Detection")
...
X_all      = X_all.values
groups_all = df["speaker_id"].astype(str).values
...
for ds in df["dataset"].unique():
    ...
    log.info(f"  {ds:<15}: {len(sub):>5} rows  "
             f"PD={(sub['label']==1).sum()}  "
             f"HC={(sub['label']==0).sum()}  "
...
D2_mask = (df["dataset"] == "voiced").values

splits = {
    "PC-GITA (ES)":  {"mask": D1_mask,  "label": "D1"},
    "VOICED (IT)":   {"mask": D2_mask,  "label": "D2"},
    "Combined":      {"mask": np.ones(len(df), dtype=bool), "label": "D1+D2"},
}

for name, info in splits.items():
    m = info["mask"]
    log.info(f"  {name:<15}: {m.sum():>5} rows  "
...
for train_name, train_info in splits.items():
    for test_name, test_info in splits.items():
        ...
        if train_name == test_name:
            ...
            auc, ci_lo, ci_hi, proba = run_within_cv(
                X_tr, y_tr, g_tr, MAIN_MODEL)
            cell_type = "CV"
        else:
        ...
        log.info(f"  [{train_name:<15} → {test_name:<15}]  "
...
log.info("")
log.info("  ── AUC Matrix ──")
log.info(f"  {'Train \\ Test':<20}" +
         "".join(f"{n:>18}" for n in split_names))
...
cross_pairs = [
    ("PC-GITA (ES)", "VOICED (IT)"),
    ("VOICED (IT)",  "PC-GITA (ES)"),
]
all_model_names = ["Logistic Regression", "Random Forest",
...
for train_name, test_name in cross_pairs:
    ...
    for mname in all_model_names:
        try:
            auc, ci_lo, ci_hi, _ = run_cross_dataset(
                X_tr, y_tr, X_te, y_te, mname)
            log.info(f"    {mname:<22}: AUC={auc:.4f}  "
                     f"95% CI=[{ci_lo:.4f}, {ci_hi:.4f}]")
            cross_rows.append({
                "train": train_name, "test": test_name,
                "model": mname, "AUC": round(auc,4),
...
for train_name, test_name in cross_pairs:
    ...
    for stage_name, thresh in zip(stage_names, thresholds):
        ...
        proba_stage = stage_models[stage_name].predict_proba(
            X_te[remaining_idx])[:, 1]
        ...
        final_proba[confident_idx] = proba_stage[confident_mask] \
            if thresh is not None else proba_stage
        handled_by[confident_idx] = stage_name

        pct = len(confident_idx) / n_test * 100
        if len(confident_idx) > 0:
            auc_stage = roc_auc_score(y_te[confident_idx],
                                       final_proba[confident_idx])
            ci_lo, ci_hi = bootstrap_auc_ci(y_te[confident_idx],
                                             final_proba[confident_idx])
        else:
            auc_stage = float("nan"); ci_lo = ci_hi = float("nan")

        log.info(f"    Stage {stage_names.index(stage_name)+1} "
        ...
        cascade_rows.append({
            "train": train_name, "test": test_name,
            "stage": stage_name, "stage_num": stage_names.index(stage_name)+1,
    ...
    log.info(f"    FULL CASCADE AUC: {auc_cascade:.4f}  "
             f"95% CI=[{ci_lo:.4f}, {ci_hi:.4f}]")
    log.info("")

    cascade_rows.append({
        "train": train_name, "test": test_name,
        "stage": "FULL_CASCADE", "stage_num": 0,
        "n_handled": n_test, "pct_handled": 100.0,
...
# ══════════════════════════════════════════════════════════════
section("6. Feature Sub-Analysis  (biomarker vs spectral vs all)")

log.info("  Answers: which feature group drives the cross-lingual signal?")
log.info("")

# Feature subsets
biomarker_feats = [f for f in feat_names if any(x in f for x in
    ["jitter_local","jitter_rap","jitter_ppq5",
     "shimmer_local","shimmer_apq3","shimmer_apq5","shimmer_apq11",
     "hnr","nhr"])]
spectral_feats  = [f for f in feat_names if any(x in f for x in
    ["mfcc","spectral","log_energy","zcr","mel","chroma",
     "praat_f0"])]
all_feats       = feat_names

subsets = {
    "Biomarkers only": biomarker_feats,
    "Spectral only":   spectral_feats,
    "All features":    all_feats,
}

log.info(f"  Biomarker features : {len(biomarker_feats)}")
log.info(f"  Spectral features  : {len(spectral_feats)}")
log.info(f"  All features       : {len(all_feats)}")
log.info("")
...
im = ax1.imshow(auc_vals, cmap="RdYlGn", vmin=0.50, vmax=1.00,
                aspect="auto")
...
for i in range(len(split_names)):
    for j in range(len(split_names)):
        ...
        if not np.isnan(val):
            ...
            ax1.text(j, i - 0.12, f"{val:.3f}",
                     ha="center", va="center",
                     fontsize=11, fontweight="bold", color=txt_color)
            ax1.text(j, i + 0.22, f"[{ci_lo:.3f},{ci_hi:.3f}]",
                     ha="center", va="center",
                     fontsize=8, color=txt_color)

diag_patch = plt.Rectangle((-0.5, -0.5), 1, 1,
                             fill=False, edgecolor="#1e293b",
                             linewidth=2.5, linestyle="--")
ax1.add_patch(diag_patch)

ax1.set_xticks(range(len(split_names)))
ax1.set_yticks(range(len(split_names)))
ax1.set_xticklabels(["Test: " + n for n in split_names],
                     fontsize=9, fontweight="bold")
ax1.set_yticklabels(["Train: " + n for n in split_names],
                     fontsize=9, fontweight="bold")
ax1.set_title(f"3×3 AUC Matrix ({MAIN_MODEL})\n"
...
colors   = {"Logistic Regression": "#2563EB", "Random Forest": "#DC2626",
            "Gradient Boosting": "#16a34a", "SVM (RBF)": "#f59e0b"}
...
for i, mname in enumerate(all_model_names):
    ...
    yerr = np.array([[a-lo, hi-a] for a, lo, hi in
                      zip(aucs, cilo, cihi)]).T
    bars = ax2.bar(x + i*w - 0.4 + w/2, aucs, w,
                   label=mname.split("(")[0].strip(),
                   color=colors.get(mname, "#94a3b8"),
                   edgecolor="white", alpha=0.9)
    ax2.errorbar(x + i*w - 0.4 + w/2, aucs, yerr=yerr,
                 fmt="none", color="#1e293b", capsize=3, linewidth=1.2)
...
ax2.set_title("Cross-Lingual AUC\n(all models, ± 95% CI)",
              fontsize=10, fontweight="bold")
...
pair_nms  = [f"{tr.split('(')[0].strip()} → {te.split('(')[0].strip()}"
             for tr, te in cross_pairs]
x2 = np.arange(len(pair_nms))
w2 = 0.8 / len(subset_nms)
sub_colors = {"Biomarkers only": "#ef4444",
              "Spectral only":   "#8b5cf6",
              "All features":    "#2563EB"}

for i, sname in enumerate(subset_nms):
    sub_s = sub_df[sub_df["subset"] == sname]
    aucs  = [sub_s[sub_s["train"]==tr][sub_s["test"]==te]["AUC"].values[0]
             if len(sub_s[(sub_s["train"]==tr) & (sub_s["test"]==te)]) > 0
             else 0.0 for tr, te in cross_pairs]
    cilo  = [sub_s[(sub_s["train"]==tr) & (sub_s["test"]==te)]["CI_lo"].values[0]
             if len(sub_s[(sub_s["train"]==tr) & (sub_s["test"]==te)]) > 0
             else 0.0 for tr, te in cross_pairs]
    cihi  = [sub_s[(sub_s["train"]==tr) & (sub_s["test"]==te)]["CI_hi"].values[0]
             if len(sub_s[(sub_s["train"]==tr) & (sub_s["test"]==te)]) > 0
             else 0.0 for tr, te in cross_pairs]
    n_f   = subsets[sname]
    yerr  = np.array([[a-lo, hi-a] for a,lo,hi in zip(aucs,cilo,cihi)]).T
    bars  = ax3.bar(x2 + i*w2 - 0.4 + w2/2, aucs, w2,
                    label=f"{sname} (n={len(n_f)})",
                    color=sub_colors[sname], edgecolor="white", alpha=0.9)
    ax3.errorbar(x2 + i*w2 - 0.4 + w2/2, aucs, yerr=yerr,
                 fmt="none", color="#1e293b", capsize=3, linewidth=1.2)
    for bar, val in zip(bars, aucs):
        if val > 0.5:
...
ax3.set_title("Feature Sub-Analysis: Which Group Drives Cross-Lingual Signal?\n"
              "(Random Forest, ± 95% CI)",
              fontsize=10, fontweight="bold")
...
stage_colors = {"Logistic Regression": "#2563EB",
...
for pair_idx, (tr, te) in enumerate(cross_pairs):
    ...
    for _, row in sub_c.iterrows():
        ...
        ax4.barh(bottom, w_bar, left=left, height=0.8,
                 color=stage_colors.get(row["stage"], "#94a3b8"),
                 edgecolor="white", alpha=0.9)
        if w_bar > 0.1:
            ax4.text(left + w_bar/2, bottom,
    ...
    ax4.text(-0.02, bottom, pair_label, ha="right", va="center",
             fontsize=9, fontweight="bold")

legend_els = [
    plt.Rectangle((0,0),1,1, fc=stage_colors["Logistic Regression"],
                  label=f"Stage 1: LR (≥70%)"),
    plt.Rectangle((0,0),1,1, fc=stage_colors["Random Forest"],
                  label=f"Stage 2: RF (≥65%)"),
    plt.Rectangle((0,0),1,1, fc=stage_colors["Gradient Boosting"],
                  label=f"Stage 3: GB (all)"),
]
...
ax4.set_title("Cascade Pipeline\n% handled per stage + AUC",
              fontsize=10, fontweight="bold")
ax4.set_yticks([])
...
for mname in all_model_names:
    ...
    voi_within = matrix_auc.loc["VOICED (IT)",  "VOICED (IT)"]

    # Cross
    pc_to_voi = sub_m[(sub_m["train"]=="PC-GITA (ES)") &
                      (sub_m["test"]=="VOICED (IT)")]["AUC"].values
    voi_to_pc = sub_m[(sub_m["train"]=="VOICED (IT)") &
                      (sub_m["test"]=="PC-GITA (ES)")]["AUC"].values

    if len(pc_to_voi) > 0:
        comparison_data.append({
            "label": f"{mname}\nES within → IT cross",
            "within": pc_within, "cross": pc_to_voi[0],
            "color": colors.get(mname, "#94a3b8"),
        })
...
bars_w = ax5.bar(x3 - 0.2, within_vals, 0.35, label="Within-dataset (CV)",
                  color=bar_colors3, edgecolor="white", alpha=0.9)
bars_c = ax5.bar(x3 + 0.2, cross_vals,  0.35, label="Cross-lingual (ES→IT)",
                  color=bar_colors3, edgecolor="white", alpha=0.5,
                  hatch="//")
...
for i, (w, c) in enumerate(zip(within_vals, cross_vals)):
    drop = w - c
    ax5.annotate(f"Δ={drop:.3f}",
                 xy=(i+0.2, c), xytext=(i, max(w,c)+0.06),
...
ax5.set_title("Within-Dataset vs Cross-Lingual AUC Degradation\n"
...
log.info("  ── Cross-Lingual AUC (PC-GITA ES → VOICED IT) ──")
for mname in all_model_names:
    sub = cross_df[(cross_df["model"]==mname) &
                   (cross_df["train"]=="PC-GITA (ES)") &
                   (cross_df["test"]=="VOICED (IT)")]
    if len(sub) > 0:
        row = sub.iloc[0]
        log.info(f"  {mname:<22}: AUC={row['AUC']:.4f}  "
                 f"95% CI=[{row['CI_lo']:.4f},{row['CI_hi']:.4f}]")

log.info("")
log.info("  ── Cross-Lingual AUC (VOICED IT → PC-GITA ES) ──")
for mname in all_model_names:
    sub = cross_df[(cross_df["model"]==mname) &
                   (cross_df["train"]=="VOICED (IT)") &
...
for sname in subsets:
    ...
    for tr, te in cross_pairs:
        ...
        if len(row) > 0:
            ...
            log.info(f"    {dir_lbl}: AUC={r['AUC']:.4f}  "
