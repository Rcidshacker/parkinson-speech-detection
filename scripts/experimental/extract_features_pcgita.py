"""
PC-GITA Standalone Feature Extraction
=======================================
Project : Speech-Based Parkinson's Disease Detection (BE Capstone)
Author  : Ruchit Das (22AM1084)

PURPOSE:
    Standalone extraction for PC-GITA (Spanish) dataset only.
    Produces features/features_pcgita_standalone.csv

    Use this when:
    - You want to re-extract PC-GITA without touching VOICED
    - Adding or verifying PC-GITA data independently
    - Feeding into per_dataset_ranking.py for individual feature ranking

DATASET:
    PC-GITA (Spanish) : ~1,318 sustained vowel files
    Source            : data/active/pc_gita
    Labels            : PD / HC (from subfolder structure)
...
HOW TO RUN:
    ...
    python scripts\\extract_features_pcgita.py

OUTPUT:
    features/features_pcgita_standalone.csv
    logs/extraction_pcgita_<timestamp>.log
"""
...
DATA_PATH  = os.path.join(BASE, "data", "active", "pc_gita")
OUTPUT_CSV = os.path.join(BASE, "features", "features_pcgita_standalone.csv")
...
LOG_FILE  = os.path.join(LOG_DIR, f"extraction_pcgita_{TIMESTAMP}.log")
...
logger = logging.getLogger("pcgita_extractor")
...
def _pcgita_speech_type(wav_path):
    """
    Identify task from any folder named in PCGITA_TASK_MAP anywhere in the path.
    Works regardless of whether audio_pd_gita_co is present as a parent folder.
    """
    for part in wav_path.replace("\\", "/").split("/"):
        task = PCGITA_TASK_MAP.get(part.lower())
        if task:
            return task
    return "unknown"
...
def main():
    logger.info("=" * 65)
    logger.info("  PC-GITA Standalone Feature Extraction")
    ...
    # Collect
    logger.info("\n[Step 1] Collecting PC-GITA sustained vowel records...")
    records = collect_pcgita(DATA_PATH)
    pd_count = sum(1 for r in records if r["label_binary"] == 1)
    hc_count = sum(1 for r in records if r["label_binary"] == 0)
    pd_subs  = len(set(r["subject_id"] for r in records if r["label_binary"] == 1))
    hc_subs  = len(set(r["subject_id"] for r in records if r["label_binary"] == 0))
    logger.info(f"  Files   : {len(records)}  PD={pd_count}  HC={hc_count}")
    logger.info(f"  Subjects: PD={pd_subs}  HC={hc_subs}  (expect ~50/50)")

    if not records:
        logger.error("No records found. Check DATA_PATH and folder structure.")
        sys.exit(1)
    ...
    results = Parallel(n_jobs=N_JOBS, prefer="threads")(
        ...
        for rec in tqdm(records, desc="PC-GITA", leave=True)
    )
    ...
    logger.info("\n  Next: run extract_features_voiced.py, then per_dataset_ranking.py")
