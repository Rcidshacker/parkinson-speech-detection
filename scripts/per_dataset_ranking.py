...
OUTPUTS:
    ...
    ├── <dataset>_feature_ranking.csv   (for each dataset)
...
# The CSV files to process
CSV_FILES = [
    os.path.join(BASE, "features", "features_sustained_a.csv"),
    os.path.join(BASE, "features", "features_opensmile_egemaps.csv")
]

# Meta columns to strictly exclude from statistical feature ranking
META_COLS = {
    "dataset", "subject_id", "language", "gender", "speech_type",
    "disease_label", "label_binary", "multiclass_label", "label",
    "updrs_total", "moca_score", "meds_status", "file",
    "rms_original", "rms_after", "rms_target", "rms_clipped", "duration_s",
    "vowel_letter", "attempt_num", "speaker_id",
    "rpde", "dfa", "ppe",  # Exclude nonlinear NaNs if present
}
...
def rank_features(df, dataset_name, log):
    """Rank all features for a single dataset using Mann-Whitney U & AUC."""
    label_col = "label_binary" if "label_binary" in df.columns else "label"
    feat_cols = [c for c in df.columns 
                 if c not in META_COLS and pd.api.types.is_numeric_dtype(df[c])]

    log.info(f"  [{dataset_name}] {len(df)} rows | PD={df[label_col].sum()} HC={(df[label_col]==0).sum()} | Features={len(feat_cols)}")

    ranking = []
    for feat in feat_cols:
        pd_vals = df[df[label_col] == 1][feat].dropna()
        hc_vals = df[df[label_col] == 0][feat].dropna()
        ...
        # Determine direction for AUC (ensure AUC >= 0.5)
        direction = "PD>HC" if pd_vals.mean() > hc_vals.mean() else "PD<HC"
        signed_feat = df[feat] if direction == "PD>HC" else -df[feat]
        valid_mask = df[feat].notna()

        try:
            auc = roc_auc_score(df[valid_mask][label_col], signed_feat[valid_mask])
        except Exception:
            continue
    ...
    rank_df = pd.DataFrame(ranking).sort_values("auc", ascending=False).reset_index(drop=True)
...
def merge_top_n_multi(rankings, top_n):
    ...
    merged_df = pd.DataFrame(rows).sort_values(
        ["dataset_count", "mean_auc_in_top"], ascending=[False, False]
...
def make_plots_multi(rankings, merged_df, summary, top_n, out_path, feature_set_name):
    ...
    # ── 1. Bar charts (Top 20) for each dataset ──
    ...
    for i, ds in enumerate(datasets):
        ...
        # Highlight features that are common across ALL datasets
    ...
    try:
        ...
        ax_venn.text(0.5, 0.5, "matplotlib-venn not installed.\nCannot render Venn diagram.", 
    ...
    ax_sweep.plot(n_vals[:len(overlaps)], overlaps, "-o", color="#7B1FA2", linewidth=2.5, label="Common in ALL")
    ax_sweep_twin = ax_sweep.twinx()
    ax_sweep_twin.plot(n_vals[:len(totals)], totals, "-s", color="#F57C00", linewidth=2.5, label="Total Merged Set")
    
    ax_sweep.axvline(top_n, color="red", linestyle="--", label=f"Current Selection (N={top_n})")
    ...
    ax_sweep_twin.set_ylabel("Total Unique Features in Merge", color="#F57C00", fontsize=10)
    ax_sweep.set_title("Cross-Dataset Feature Stability\n(Helps determine optimal N)", fontweight="bold", fontsize=12)
...
def process_feature_set(csv_path, top_n):
    feature_set_name = os.path.basename(csv_path).replace(".csv", "").replace("features_", "")
    
    out_dir = os.path.join(BASE, "results", "final", 
                           f"ranking_{feature_set_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
    os.makedirs(out_dir, exist_ok=True)
    
    # Init logger for this specific run
    log_file = os.path.join(out_dir, "ranking_report.log")
    log = logging.getLogger(feature_set_name)
    log.setLevel(logging.INFO)
    # Clear existing handlers if looping
    if log.hasHandlers():
        log.handlers.clear()
    ...
    for _, row in merged_df.head(15).iterrows():
        ...
        log.info(f"  {star}{row['feature']:<30} | Present in {row['dataset_count']}/{len(datasets)} datasets | Mean Top-AUC: {row['mean_auc_in_top']:.4f}")

    # 5. Visualizations
    log.info("\n[STEP 5] Generating Visual Reports...")
    plot_path = os.path.join(out_dir, "ranking_plots.png")
    try:
        make_plots_multi(rankings, merged_df, summary, top_n, plot_path, feature_set_name)
...
def main():
    ...
    parser.add_argument("--top_n", type=int, default=15,
                        help="Number of top features to extract per dataset (default: 15)")
    args = parser.parse_args()

    os.makedirs(LOG_DIR, exist_ok=True)

    # Loop through our target CSVs
