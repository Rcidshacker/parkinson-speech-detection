"""
Sustained Vowel "a" Feature Extraction Pipeline  — 16 kHz
==========================================================
Project : Speech-Based Parkinson's Disease Detection (BE Capstone)
Author  : Ruchit Das (22AM1084)

SAMPLE RATE : 16,000 Hz
    Nyquist  : 8,000 Hz  (captures F0, F1–F4 formants, high-freq shimmer/HNR harmonics,
                           turbulence/breathiness cues — standard for speech research)
    N_FFT    : 512 samples  (32 ms window at 16 kHz — same temporal resolution as 8 kHz baseline)
    HOP      : 256 samples  (50% overlap)
    NOTE     : wav2vec 2.0 requires 16 kHz — this output is directly usable for
               foundation model fine-tuning without any resampling step.
...
OUTPUT:
    features/features_sustained_a_16k.csv          -- clean, deduped feature matrix
    features/dedup_report_sustained_a_16k.csv      -- log of removed duplicate pairs
    logs/extraction_sustained_a_16k_<ts>.log       -- full run log

HOW TO RUN:
    venv\\Scripts\\activate
    python extract_features_sustained_a_16k.py
"""
...
OUTPUT_CSV   = os.path.join(BASE, "features", "features_sustained_a_16k.csv")
DEDUP_REPORT = os.path.join(BASE, "features", "dedup_report_sustained_a_16k.csv")
...
N_MFCC           = 13
N_FFT            = 512   # 32ms at 16 kHz  (same temporal resolution as 8 kHz baseline)
HOP_LENGTH       = 256   # 50% overlap
DELTA_WIDTH      = 9
F0_MIN           = 80
F0_MAX           = 300
FORMANT_MAX_FREQ = 5500  # Nyquist at 16 kHz = 8000 Hz; 5500 safely captures F1–F4

# Feature quality thresholds
# At 16 kHz + N_FFT=512, the larger FFT gives more spectral resolution,
# so near-zero variance features are even less likely. Threshold kept for safety.
...
LOG_FILE  = os.path.join(LOG_DIR, f"extraction_sustained_a_16k_{TIMESTAMP}.log")
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("  SUSTAINED VOWEL 'A' FEATURE EXTRACTION PIPELINE  [16 kHz]")
    logger.info("=" * 80)
